...
impl ATClientActor {
    fn new(
        ...
        let delete_mms_notification = config.notification_config.delete_mms_notification;
        let async_sms_delete_after_forward = sms_delete_after_forward;
        let async_delete_mms_notification = delete_mms_notification;
        ...
        tokio::spawn(async move {
            let mut async_handlers: Vec<Box<dyn MessageHandler>> = vec![
                ...
                Box::new(NewSMSHandler::new(
                    async_sms_delete_after_forward,
                    async_delete_mms_notification,
                )),
        ...
        let handlers: Vec<Box<dyn MessageHandler>> = vec![
            Box::new(CallHandler),
            Box::new(MemoryFullHandler),
            Box::new(NewSMSHandler::new(sms_delete_after_forward, delete_mms_notification)),
            Box::new(NdisStatHandler),
            Box::new(PDCPDataHandler),
            Box::new(NetworkSignalHandler::new()),
        ];
