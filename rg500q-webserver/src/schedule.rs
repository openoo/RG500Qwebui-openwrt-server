...
pub async fn monitor_loop(client: Arc<ATClient>, config: ScheduleConfig) {
    ...
    debug!("  Timeout: {}s", config.timeout);
...
async fn unlock_all(client: &ATClient, config: &ScheduleConfig) -> Result<()> {
    ...
    debug!("Step 2: Restore RG500Q LTE bands...");
    send_command(client, &format!("{}\r\n", rg500q::lte_cell_unlock_command())).await?;
    sleep(Duration::from_secs(1)).await;
    send_command(client, &format!("{}\r\n", rg500q::lte_unlock_command())).await?;
    sleep(Duration::from_secs(1)).await;

    debug!("Step 3: Restore RG500Q NR bands...");
    send_command(client, &format!("{}\r\n", rg500q::nr_cell_unlock_command())).await?;
    sleep(Duration::from_secs(1)).await;
    send_command(client, &format!("{}\r\n", rg500q::nr_unlock_command())).await?;
    sleep(Duration::from_secs(1)).await;

    debug!("Step 4: Restore network mode preference...");
    send_command(client, &format!("{}\r\n", rg500q::mode_auto_command())).await?;
    sleep(Duration::from_secs(1)).await;

    // Exit airplane mode
    if config.toggle_airplane {
        debug!("Step 5: Exit airplane mode...");
...
fn build_lte_command(lock_type: u8, bands: &[&str], arfcns: &str, pcis: &str) -> String {
    // RG500Q: band lock uses AT+QNWPREFCFG. Frequency/cell lock is best-effort
    // through AT+QNWLOCK because firmware/operator support differs across RG50x builds.
    
    if lock_type == 3 {
        return format!("AT+QNWPREFCFG=\"lte_band\",{}\r\n", bands.join(":"));
    } else if lock_type == 1 {
        ...
        if bands.len() != arfcn_list.len() {
            ...
            return format!("{}\r\n", rg500q::lte_unlock_command());
        }
        let first_earfcn = arfcn_list.first().copied().unwrap_or("");
        return format!("AT+QNWLOCK=\"common/lte\",1,{},0\r\n", first_earfcn);
    } else if lock_type == 2 {
        let arfcn_list: Vec<&str> = arfcns.split(',').map(|s| s.trim()).filter(|s| !s.is_empty()).collect();
        ...
        let first_earfcn = arfcn_list.first().copied().unwrap_or("");
        let first_pci = pci_list.first().copied().unwrap_or("");
        return format!("AT+QNWLOCK=\"common/lte\",2,{},{}\r\n", first_earfcn, first_pci);
    }

    format!("{}\r\n", rg500q::lte_unlock_command())
}

fn build_nr_command(lock_type: u8, bands: &[&str], arfcns: &str, _scs_types: &str, pcis: &str) -> String {
    // RG500Q: band lock uses AT+QNWPREFCFG. NR point/cell lock uses QNWLOCK
    // best-effort syntax and may depend on module firmware.

    if lock_type == 3 {
        return format!("AT+QNWPREFCFG=\"nr5g_band\",{}\r\n", bands.join(":"));
    } else if lock_type == 1 {
        let arfcn_list: Vec<&str> = arfcns.split(',').map(|s| s.trim()).filter(|s| !s.is_empty()).collect();
        if bands.len() != arfcn_list.len() {
            warn!("NR Frequency Lock: Band count ({}) != ARFCN count ({}), unlocking", bands.len(), arfcn_list.len());
            return format!("{}\r\n", rg500q::nr_unlock_command());
        }
        let first_arfcn = arfcn_list.first().copied().unwrap_or("");
        return format!("AT+QNWLOCK=\"common/5g\",1,{},0\r\n", first_arfcn);
    } else if lock_type == 2 {
        let arfcn_list: Vec<&str> = arfcns.split(',').map(|s| s.trim()).filter(|s| !s.is_empty()).collect();
        let pci_list: Vec<&str> = pcis.split(',').map(|s| s.trim()).filter(|s| !s.is_empty()).collect();

        if bands.len() != arfcn_list.len() || arfcn_list.len() != pci_list.len() {
            warn!("NR Cell Lock: Count mismatch (Band:{}, ARFCN:{}, PCI:{}), unlocking", bands.len(), arfcn_list.len(), pci_list.len());
            return format!("{}\r\n", rg500q::nr_unlock_command());
        }
        let first_arfcn = arfcn_list.first().copied().unwrap_or("");
        let first_pci = pci_list.first().copied().unwrap_or("");
        return format!("AT+QNWLOCK=\"common/5g\",1,{},{}\r\n", first_arfcn, first_pci);
    }
