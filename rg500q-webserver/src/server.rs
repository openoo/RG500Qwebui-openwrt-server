            ...
            Some(result) = rx.next() => {
                match result {
                    Ok(msg) => {
                         ...
                         if let Some(kv) = cmd_str.trim().strip_prefix("SET_CONFIG:") {
                             if let Some((key, value)) = kv.split_once('=') {
                                 ...
                                 let success = if allowed_keys.contains(&key.as_str()) {
                                     let uci_key = format!("rg500q-webserver.config.{}={}", key, normalized_value);
                         ...
                         if let Some(translated) = rg500q::translate_legacy_command(&cmd_str) {
                             info!("Translated legacy AT command for RG500Q: {} -> {}", cmd_str.trim(), translated);
                             cmd_str = translated;
                         }
