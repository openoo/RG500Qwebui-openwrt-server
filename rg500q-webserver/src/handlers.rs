...
/// 处理数据通道状态 URC，实时感知拨号连接状态变化。
/// 兼容旧 MT5700 的 ^NDISSTAT，同时支持 Quectel RG500Q 的 QNET/QIURC 状态。
...
impl MessageHandler for NdisStatHandler {
    fn can_handle(&self, line: &str) -> bool {
        line.starts_with("^NDISSTAT:")
            || line.starts_with("+QNETDEVSTATUS:")
            || line.starts_with("+QNETDEVCTL:")
            || line.contains("+QIURC: \"pdpdeact\"")
    }
    async fn handle(
        &self,
        line: &str,
        ...
        let mut pdp_type = String::new();
        let (connected, stat_str) = if line.contains("+QIURC: \"pdpdeact\"") {
            (false, "disconnected")
        } else if line.starts_with("+QNETDEVSTATUS:") || line.starts_with("+QNETDEVCTL:") {
            let data = line
                .trim_start_matches("+QNETDEVSTATUS:")
                .trim_start_matches("+QNETDEVCTL:")
                .trim();
            let parts: Vec<&str> = data.split(',').map(|s| s.trim()).collect();
            let stat = parts.iter().rev().find_map(|s| s.parse::<u8>().ok());
            match stat {
                Some(1) => (true, "connected"),
                Some(0) => (false, "disconnected"),
                _ => {
                    warn!("+QNETDEVSTATUS: unrecognized format: {}", line);
                    return Ok(());
                }
            }
        } else {
        ...
        if connected {
            info!("RG500Q data channel established ({})", pdp_type);
        } else {
            warn!("RG500Q data channel lost (type={})", pdp_type);
...
impl MessageHandler for NetworkSignalHandler {
    fn can_handle(&self, line: &str) -> bool {
        line.contains("^CERSSI:")
            || line.contains("^HCSQ:")
            || line.contains("+QCSQ:")
            || line.contains("+QENG:")
            || line.contains("+QNWINFO:")
    }
    async fn handle(
        &self,
        line: &str,
        ...
        } else if line.contains("+QCSQ:") {
            // +QCSQ: "LTE",-67,-95,173,-12
            // +QCSQ: "NR5G",-84,-100,130,-10
            let replaced = line.replace("+QCSQ:", "");
            let parts: Vec<&str> = replaced.split(',').map(|s| s.trim()).collect();
            if let Some(mode) = parts.first() {
                let mode = mode.trim_matches('"');
                if mode != "NOSERVICE" {
                    current_sys_mode = Some(mode.to_string());
                    if let Some(rsrp_part) = parts.get(2) {
                        if let Ok(rsrp) = rsrp_part.parse::<i32>() {
                            current_rsrp = Some(rsrp);
                        }
                    }
                }
            }
        } else if line.contains("+QENG:") || line.contains("+QNWINFO:") {
            current_sys_mode = Some("RG500Q".to_string());
        }
        ...
        if should_notify {
            // Query detailed info
            let cmd = "AT+QENG=\"servingcell\"".to_string();
            let (tx, rx) = oneshot::channel();
            if let Err(_) = cmd_tx.send((cmd, tx)).await {
                return Ok(());
            }
            ...
            if let Ok(response) = rx.await {
                if let Some(data) = response.data {
                    ...
                    if message.is_empty() && data.contains("+QENG:") {
                        message = format!(
                            "RG500Q 信号/小区信息变动\n时间: {}\n{}",
                            chrono::Local::now().format("%Y-%m-%d %H:%M:%S"),
                            data.trim()
