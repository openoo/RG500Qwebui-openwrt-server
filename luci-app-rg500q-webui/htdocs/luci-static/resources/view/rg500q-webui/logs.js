...
return view.extend({
	load: function() {
		return uci.load('rg500q-webserver').then(function() {
			var notifyPersist = uci.get('rg500q-webserver', 'config', 'notify_log_persist') == '1';
			var syslogPersist = uci.get('rg500q-webserver', 'config', 'sys_log_persist') == '1';
			var notifyPath = notifyPersist ? '/var/log/at-notifications.log' : '/tmp/at-notifications.log';
			var sysPath    = syslogPersist  ? '/var/log/rg500q-webserver.log'    : '/tmp/rg500q-webserver.log';
